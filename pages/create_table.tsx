...
function CreateTable(props: { close: () => void }) {
    ...
    const handleTableNameTextChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const handleCreateNewTable = async () => {
        ...
        for (let i = 0; i < data.length; i++) {
            if (names.has(data[i].name)) {
                handleError("Column names must be unique [" + data[i].name + "] is used more than once");
                return;
            }
